...
namespace AdventOfCode_2022
{
        ...
        static public void Run(string day, Delegate input, Func<object> solve1, Func<object> solve2)
    {
        ...
        var res1 = solve1();
        sw.Stop();
        Console.WriteLine($"Day {day} - Part 1: {sw.ElapsedMilliseconds,6} ms, {res1}");
        var p1 = sw.ElapsedMilliseconds;
        sw.Restart();
        var res2 = solve2();
        sw.Stop();
        Console.WriteLine($"Day {day} - Part 2: {sw.ElapsedMilliseconds,6} ms, {res2}");
                ...
                foreach (var key in keys)
                {
                    try
                    {
                        ...
                        if (!int.TryParse(key, out int dayNum))
                            continue;
                        var url = $"https://adventofcode.com/2022/day/{dayNum}";
